...
print(xtable(error_estimate_by_race_ethnicity_bin, digits = 3),
      file = "paper/figures/error_estimate_by_race_ethnicity_bin.tex",
...
error_estimate_by_race_ethnicity <- census_tidy %>%
  filter(sf != 0) %>%
  group_by(ethnicity_race_abbrev) %>%
  rename('Ethnicity/Race Category' = ethnicity_race_abbrev) %>%
  summarize(`Zero` = mean(pct_change == -100, na.rm = TRUE),
            `Less Than Half of True Population` = mean(pct_change == -100, na.rm = TRUE),
            `Twice the True Population` = mean(pct_change > 200, na.rm = TRUE),
            `Ten Times the True Population` = mean(pct_change > 1000, na.rm = TRUE))
...
# write this to a tex file as well
print(xtable(nonzero_estimate_by_race_ethnicity_bin, digits = 3),
      file = "paper/figures/nonzero_estimate_by_race_ethnicity_bin.tex",
      include.rownames = FALSE,
      hline.after = c(0, 1))


# calculate this by race/ethnicity category as well
nonzero_estimate_by_race_ethnicity <- census_tidy %>%
  group_by(ethnicity_race_abbrev) %>%
  rename('Ethnicity/Race Category' = ethnicity_race_abbrev) %>%
  summarize(`Estimated Nonzero Count` = mean(pct_change == Inf, na.rm = TRUE))  

nonzero_estimate_by_race_ethnicity

# save to file
write_csv(nonzero_estimate_by_race_ethnicity,
          path = "data/summaries/nonzero_estimate_by_race_ethnicity.csv")

# write this to a tex file as well
print(xtable(nonzero_estimate_by_race_ethnicity, digits = 3),
      file = "paper/figures/nonzero_estimate_by_race_ethnicity.tex",
      include.rownames = FALSE,
      hline.after = c(0, 1, 7))
